...
int main(int argc, char *argv[]) {
    ...
    while(run_status) {
        printf("please input message you want to input: ");
        fgets(message_inst.content, BUFFSIZE, stdin);
        message_inst.message_type = STRING;

        if(msgsnd(message_id, (void *)&message_inst, BUFFSIZE, 0) == MESSAGE_ERRNO) {
